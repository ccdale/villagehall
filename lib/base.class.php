...
class Base
{
  ...
  {
    $this->debug("ValidInt: input: '$int'");
    $tmp=$int + 0;
    $this->debug("ValidInt: testing against (input+0): '$tmp'");
    if($tmp===$int){
      $this->debug("ValidInt: $int is a valid int");
      return true;
    }
    $this->debug("ValidInt: '$int' is not a valid int - it is a " . gettype($int));
    return false;
  }/*}}}*/
